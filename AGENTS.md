...
| `valo-codec-apple` | Apple's system codecs, through ImageIO as a `Decoder`: frames rasterised into IOSurface-backed CoreVideo buffers that Metal samples directly (no readback), CPU pixels when that is unavailable. Animations included — ImageIO composites them itself |
